...
Format: `sort SORT_COMPARATOR [SORT_COMPARATOR] [SORT_COMPARATOR] ...` +

****
* Format of a `SORT_COMPARATOR`: `(n|name|d|due|p|priority|f|frequency)(<|>)` OR `(t|tag)(<|>){TAG1 TAG2 TAG3 ...}`
* In the above format `|` refers to an OR. `TAG1`, `TAG2` and `TAG3` are shown as example of tags that the user can input.
...
image::userguide/sortdemo_1.png[SortDemo1, width="600"]
